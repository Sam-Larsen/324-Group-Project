...
public class Graph {
  ...
  private void generateLattice() {
    visualGraph = new SingleGraph("Network Simulation");
    visualGraph.setAttribute("ui.stylesheet", "url(stylesheet.css)");
    adjacencyList.clear();
    for (int i = 0; i < size; i++) {
      visualGraph.addNode(Integer.toString(i));
  ...
  private void checkSizeCompatibility1D() {
    if (size < SIZE_MIN_1D) {
      throw new IllegalArgumentException(
        "Size must be at least " + SIZE_MIN_1D + " for a " + type + " lattice."
      );
    }
  }
  ...
  private void checkSizeCompatibility2D() {
    if (size < SIZE_MIN_2D) {
      throw new IllegalArgumentException(
        "Size must be at least " + SIZE_MIN_2D + " for a " + type + " lattice."
      );
    }
    int side = (int) Math.sqrt(size);
    if (side * side != size) {
      throw new IllegalArgumentException(
        "Size must be a perfect square for a " + type + " lattice."
      );
    }
  }
  ...
  private void addEdge(int node1, int node2) {
    ...
    Edge edge = visualGraph.getEdge(edgeId);
